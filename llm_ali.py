...
def generate_title_and_summary(content):
    ...
    prompt_text = f"""为以下新闻内容：
        ...
        2 输出一段对这些热点新闻的简要总结：总数不超过200字，不同的总结内容请换行输出（用markdown有序列表格式）；
    ...
    if LOCAL_DEV:
        ...
        title_and_summary_path = os.path.join(os.path.dirname(__file__), "test", "llm-ali", "title_and_summary.md")
        try:
...
if __name__ == "__main__":
    content = ""
    # 尝试从本地文件读取内容
    summary_file = os.path.join(os.path.dirname(__file__), "test", "llm-ali", "latest_summary.md")
    try:
